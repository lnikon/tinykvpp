...
class random_access_file_t
{
  public:
    ...
    auto operator=(random_access_file_t &&other) noexcept -> random_access_file_t &;

    random_access_file_t(const random_access_file_t &) = delete;
    auto operator=(const random_access_file_t &) -> random_access_file_t & = delete;
    ...
    [[nodiscard]] auto read(ssize_t offset, char *buffer, size_t size) const noexcept
        -> std::expected<std::size_t, file_error_t>;
