...
std::uint32_t random_height(skiplist *sl) noexcept {
  ...
  std::uniform_int_distribution<std::uint32_t> dist(0, sl->branching_factor_ - 1);
...
skiplist_node *create_skiplist_node(core::arena *arena, std::string_view key, std::string_view value,
                                    std::uint32_t height) noexcept {
  const std::uint64_t size = sizeof(skiplist_node) + height * sizeof(skiplist_node *) + key.size() + value.size();

  void *buffer = core::arena_allocate(arena, size, alignof(skiplist_node));
  std::memset(buffer, 0, size);

  skiplist_node *node = static_cast<skiplist_node *>(buffer);
  ...
  std::memcpy(reinterpret_cast<char *>(node->forward() + height) + key.size(), value.data(), value.size());

  return node;
}

skiplist *create_skiplist(core::arena *arena, std::uint32_t max_height, std::uint32_t branching_factor) noexcept {
...
skiplist_node *skiplist_search(skiplist *sl, std::string_view key) noexcept {
  ...
  for (std::int32_t current_level = static_cast<std::int32_t>(sl->max_height_ - 1); current_level >= 0;
       current_level--) {
    const std::uint64_t current_level_u64 = static_cast<std::uint64_t>(current_level);
...
skiplist_node *skiplist_insert(skiplist *sl, std::string_view key, std::string_view value) noexcept {
  ...
  if (current_node->forward()[0] != nullptr && current_node->forward()[0]->key() == key) {
    return current_node->forward()[0];
  }

  const std::uint32_t new_height = random_height(sl);
  if (new_height > sl->current_height_) {
    for (std::uint32_t level = sl->current_height_; level < new_height; level++) {
      updates[level] = sl->head_;
    }
    sl->current_height_ = new_height;
  }

  skiplist_node *new_node = create_skiplist_node(sl->arena_, key, value, new_height);
...
skiplist_iter create_skiplist_iter(skiplist_node *node) noexcept { return skiplist_iter{.current_ = node}; }

bool skiplist_iter_valid(skiplist_iter *sl) noexcept { return sl->current_ != nullptr; }
