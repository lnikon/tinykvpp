...
random_access_file_t::random_access_file_t(
    random_access_file_t &&other) noexcept
{
  if (this == &other)
  {
    return;
    ...
    -> std::expected<ssize_t, file_error_t>
{
  ...
  auto iov = iovec{.iov_base = const_cast<char *>(data.data()),
                   .iov_len = data.size()};
...
auto random_access_file_t::read(size_t offset,
                                char  *buffer,
...
auto random_access_file_t::size() const noexcept
...
auto random_access_file_t::reset() noexcept -> std::expected<void, file_error_t>
{
  if (ftruncate(m_fd, 0) != 0)
  {
    return std::unexpected(
        file_error_t::from_errno(file_error_code_k::truncate_failed,
                                 errno,
                                 "File truncate operation failed"));
  }
...
auto random_access_file_t::stream() noexcept
 ...
 */
auto random_access_file_builder_t::build(fs::path_t                 path,
                                         posix_wrapper::open_flag_k openFlags)
    -> std::expected<random_access_file_t, file_error_t>
{
  ...
  int fdes = open(path.c_str(),
                  posix_wrapper::to_native(openFlags),
