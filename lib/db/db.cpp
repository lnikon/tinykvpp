 ...
 */
db_t::db_t(const config::shared_ptr_t config) : m_config{config}, m_pSegmentStorage{lsmtree::segments::storage::make_shared()}, m_lsmTree{config} {}
...
void db_t::put(const structures::lsmtree::key_t &key, const structures::lsmtree::value_t &value) { m_lsmTree.put(key, value); }

std::optional<record_t> db_t::get(const structures::lsmtree::key_t &key) const { return m_lsmTree.get(key); }
