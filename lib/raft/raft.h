...
class raft_node_grpc_client_t
{
  public:
    ...
    auto operator=(const raft_node_grpc_client_t &)
        -> raft_node_grpc_client_t & = delete;

    raft_node_grpc_client_t(raft_node_grpc_client_t &&) = default;
    auto operator=(raft_node_grpc_client_t &&)
        -> raft_node_grpc_client_t & = default;

    auto appendEntries(const AppendEntriesRequest &request,
                       AppendEntriesResponse      *response) -> bool;
    auto requestVote(const RequestVoteRequest &request,
...
class consensus_module_t : public RaftService::Service
{
  ...
  private:
    ...
    void sendAppendEntriesRPC(raft_node_grpc_client_t &client,
                              std::vector<LogEntry>    logEntries);
    auto onSendAppendEntriesRPC(raft_node_grpc_client_t     &client,
                                const AppendEntriesResponse &response) noexcept
        -> bool;
    // --------
    ...
    void startElection();
    void sendRequestVoteRPCs(const RequestVoteRequest &request,
                             std::uint64_t             newTerm);
    // --------

    // ---- Constant utility methods ----
    [[nodiscard]] uint32_t getLastLogIndex() const
        ABSL_SHARED_LOCKS_REQUIRED(m_stateMutex);
    [[nodiscard]] uint32_t getLastLogTerm() const
        ABSL_SHARED_LOCKS_REQUIRED(m_stateMutex);
    [[nodiscard]] auto     hasMajority(uint32_t votes) const -> bool;
    [[nodiscard]] uint32_t getLogTerm(uint32_t index) const
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_stateMutex);
    [[nodiscard]] uint32_t findMajorityIndexMatch() const
        ABSL_SHARED_LOCKS_REQUIRED(m_stateMutex);
    [[nodiscard]] auto waitForMajorityReplication(uint32_t logIndex) const
        -> bool;
    // --------

    // ---- Persistent state management ----
    // TODO(lnikon): Move into separate class
    [[nodiscard]] bool initializePersistentState()
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_stateMutex);
    [[nodiscard]] bool
    updatePersistentState(std::optional<std::uint32_t> commitIndex,
                          std::optional<std::uint32_t> votedFor)
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_stateMutex);

    [[nodiscard]] bool flushPersistentState()
        ABSL_EXCLUSIVE_LOCKS_REQUIRED(m_stateMutex);
    ...
    std::unordered_map<id_t, uint32_t>
        m_matchIndex ABSL_GUARDED_BY(m_stateMutex);
    std::unordered_map<id_t, uint32_t>
        m_nextIndex ABSL_GUARDED_BY(m_stateMutex);
