...
template <typename TEntry> class replicated_log_storage_impl_t
{
  public:
    ...
    replicated_log_storage_impl_t(
        file_log_t log, std::shared_ptr<raft::consensus_module_t> pConsensusModule) noexcept;
    ...
    [[nodiscard]] auto reset_last_n(std::size_t n) -> bool
    {
        ...
        // TODO(lnikon): Not clear to handle the WAL inside the consensus module or whether should
        // it be touched here, because from the callers perspective, only in-memory log matters.
...
class replicated_log_storage_builder_t final
{
  public:
    ...
    [[nodiscard]] auto build(replicated_log_storage_impl_t<TEntry>::file_log_t log,
                             std::shared_ptr<raft::consensus_module_t> pConsensusModule) noexcept
        -> std::optional<replicated_log_storage_impl_t<TEntry>>
    {
...
template <typename TEntry>
replicated_log_storage_impl_t<TEntry>::replicated_log_storage_impl_t(
    replicated_log_storage_impl_t &&other) noexcept
...
auto replicated_log_storage_impl_t<TEntry>::operator=(
    replicated_log_storage_impl_t<TEntry> &&other) noexcept -> replicated_log_storage_impl_t &
{
    if (this == &other)
    {
        return *this;
...
[[nodiscard]] auto replicated_log_storage_impl_t<TEntry>::append(entry_type entry) noexcept -> bool
{
    ...
    // TODO(lnikon): Figure out command/key/value part :)
    // LogEntry logEntry;
    // logEntry.set_command(command);
    // logEntry.set_key(key);
    // logEntry.set_value(value);
    //
    // if (!m_pConsensusModule->replicate(std::move(logEntry)))
    // {
    //     spdlog::debug("replicated_log_storage_t: Unable to replicate the log entry. logEntry={}",
    //                   logEntry.DebugString());
    //     return false;
    // }
